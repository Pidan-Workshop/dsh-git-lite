...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function BranchChip(props) {
					...
					function tick() {
						api('/status', { sessionId: sessionId })
							...
							.catch(function (err) {
								if (!alive) return
								setBrief(null)
								// session-unknown 是**尚未就绪**（切会话时宿主还没把会话载入），
								// 不是「这里没有仓库」的权威答复，所以不显示弱化胶囊、只等就绪。
								var code = err && err.code
								setBriefErr(code && code !== 'session-unknown' ? err.message : null)
			...
			// 有仓库：正常胶囊。
			// 宿主明确回答「这里没有可用仓库」：显示一个弱化胶囊，把原因放进 tooltip。
			//   之前这里是静默 return null —— 用户无法区分「没有仓库」和「插件坏了」。
			// 还没有权威答复（加载中 / 网络问题）：不显示，避免闪一个假状态。
			if (!brief || !brief.branch) {
				if (!briefErr) return null
				return h(
					'button',
					{
						type: 'button',
