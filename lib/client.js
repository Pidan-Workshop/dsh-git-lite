 ...
 *   3. conversation.input.dock     —— 输入框上方的分支胶囊，点击打开面板
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 当前会话 id：与官方一致，订阅 sessions.list 的 observable 快照。 */
		...
		function GitTabBody(props) {
			...
			function refreshStatus() {
				return call('/status', { sessionId: sessionId })
					.then(function (value) {
						setStatus(value)
						setError(null)
					})
			...
			React.useEffect(
				function () {
					...
					call('/diff', { sessionId: sessionId, path: selected.path, staged: selected.staged })
						.then(function (value) {
			...
			function onStage(file) {
				run('stage', call('/stage', { sessionId: sessionId, path: file.path })).then(afterMutation)
			}

			function onUnstage(file) {
				run('unstage', call('/unstage', { sessionId: sessionId, path: file.path })).then(afterMutation)
			}

			function onPush() {
				run('push-preview', call('/push/preview', { sessionId: sessionId })).then(function (value) {
			...
			function onPushConfirm() {
				...
				run('push-confirm', call('/push/confirm', { sessionId: sessionId, token: token })).then(function (value) {
					if (value) {
						setConfirmPush(null)
			...
			function onGenerate() {
				run('message', call('/message', { sessionId: sessionId })).then(function (value) {
					if (value && value.message) setMessage(value.message)
				})
			}

			function onCommit() {
				run('commit', call('/commit', { sessionId: sessionId, message: message, stageAll: stageAll })).then(
					function (value) {
						if (value) {
							setMessage('')
			...
			function onFetch() {
				run('fetch', call('/fetch', { sessionId: sessionId })).then(function (value) {
					if (value) {
						ok(value.output)
						return refreshStatus()
					}
			...
			function onPull() {
				run('pull', call('/pull', { sessionId: sessionId })).then(function (value) {
					if (value) {
			...
			function onStash(action) {
				run('stash', call('/stash', { sessionId: sessionId, action: action })).then(function (value) {
					if (value) {
			...
			function onCheckout(branch) {
				run('checkout', call('/checkout', { sessionId: sessionId, branch: branch })).then(function (value) {
					if (value) {
						ok(branch)
						return afterMutation()
					}
			...
			function loadBranches() {
				...
				call('/branches', { sessionId: sessionId })
					.then(function (value) {
						setBranches(value)
					})
					.catch(report)
		...
		function BranchChip(props) {
			...
			React.useEffect(
				function () {
					...
					function tick() {
						if (typeof document !== 'undefined' && document.hidden) return
						call('/status', { sessionId: sessionId })
							.then(function (v) {
								if (alive) setBrief(v)
							})
							.catch(function () {
		...
		function apply(ctx) {
			...
			// 输入框上方的分支胶囊：给面板一个常驻入口（工作区启动卡只在欢迎页出现）。
			ctx.inject(['slots', 'sessions', 'locale', 'sidebarRight'], function (scope) {
				try {
					...
					console.warn('dsh-git-lite: branch chip registration failed', err)
				}
			})
		}
