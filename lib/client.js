	...
	return h(
		...
		function GitTabBody(props) {
			...
			function commitListContent() {
				...
				return [].concat(
					logState.commits.map(commitRow),
					logState.hasMore
						? [
								h(
									'button',
		...
		function apply(ctx) {
			...
			ctx.inject(['sidebarRightTabs'], function (scope) {
				try {
					...
					console.warn('dsh-git-lite: tab type registration failed', err)
				}
			})
			...
			ctx.inject(['slots', 'sessions', 'locale'], function (scope) {
				try {
					...
					console.warn('dsh-git-lite: pane body registration failed', err)
				}
				try {
					claimSeat(
						scope,
					...
					console.warn('dsh-git-lite: pane title registration failed', err)
				}
			})
