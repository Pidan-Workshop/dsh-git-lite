	...
	factory: (require) => {
			...
			return h(
				'button',
				{
					...
					style: {
						...
						fontFamily: 'inherit',
						lineHeight: '18px',
						color: props.primary ? '#fff' : theme.fg,
				...
				hasNode
					? h('span', {
							...
							style: {
								...
								left: '50%',
								top: '50%',
								// 从节点右缘画到 gutter 右缘：既贴着节点，又不越界

								// （越界就会压到标题文字 —— 那正是标题曾经多出 8px 内边距的原因）
		...
		function ensurePanelStyle() {
			...
			el.textContent = [
				...
				// 提交卡片悬停：颜色与 theme.panel 一致（浅色 rgba(0,0,0,.03) / 深色 rgba(255,255,255,.04)）
				'[data-git-lite-commit]{transition:background .12s ease}',
				'[data-git-lite-commit]:hover{background:rgba(0,0,0,.035)}',
				'@media (prefers-color-scheme: dark){[data-git-lite-commit]:hover{background:rgba(255,255,255,.055)}}'
			].join('')
				...
				mode === 'changes'
					? h(
						...
						h(
							...
							h(Btn, { t: t, theme: theme, label: t.generate, busy: busy === 'message', disabled: busy !== null, onClick: onGenerate }),
							h(Btn, { t: t, theme: theme, label: t.commit, primary: true, busy: busy === 'commit', disabled: busy !== null, onClick: onCommit }),
							h(Btn, { t: t, theme: theme, label: t.delegate, disabled: busy !== null, onClick: onDelegate, title: t.delegate })
						)
					)
					: null,
