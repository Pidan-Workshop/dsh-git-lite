...
function makeTheme() {
	return {
		...
		accent: tok('--dsw-alias-brand-primary', '#0969da'),
...
// ────────────────────── 避开第三方固定浮层 ──────────────────────
		...
		function ensurePanelStyle() {
			...
			el.textContent = [
				...
				'[data-git-lite-btn][data-primary]:not(:disabled):hover{background:var(--dsw-alias-button-primary-hover,var(--dsw-alias-brand-primary,#0969da))}',
				'[data-git-lite-btn]:disabled{cursor:default}',
				// 焦点环：键盘可达性（:focus-visible 只在键盘操作时出现，不干扰鼠标点击）
				'[data-git-lite-btn]:focus-visible,[data-git-lite-seg]:focus-visible,[data-git-lite-chip]:focus-visible,[data-git-lite-hoverable]:focus-visible{outline:2px solid var(--dsw-alias-brand-primary,#0969da);outline-offset:1px}',
