	...
	factory: (require) => {
		...
		var DICT = {
			zh: {
				...
				stashPush: '储藏改动',
				stageAllBtn: '全部暂存',
				stageAllBtnHint: '把工作区全部改动（含未跟踪文件与删除）加入暂存区：git add -A',
				unstageAllBtn: '全部取消暂存',
				unstageAllBtnHint: '把暂存区清空、改动退回工作区：git restore --staged',
				stagedAllCount: '已全部暂存（{n} 个文件）',
				stagedNone: '没有可暂存的改动',
				unstagedAll: '已全部取消暂存',
				conflictGroup: '{n} 个文件处于冲突状态',
				conflictLockedHint: '冲突未解决：解决之前不能暂存或取消暂存（git add 会把冲突标记当成解决结果）',
				conflictBlocked: '有冲突文件，先解决冲突再批量操作',
				resolveConflicts: 'Agent 解决冲突',
				resolveConflictsHint: '把冲突交给当前会话的 Agent 解决：它能看到完整对话上下文。它只解决冲突，不会替你提交',
				resolvingDelegated: '已把冲突交给当前会话的 Agent 解决',
				stashPop: '恢复储藏',
				...
				commitBlockedHint: '有未解决的冲突，git 不允许提交 —— 先解决冲突，或点冲突组头的「Agent 解决冲突」',
				switching: '切换中…',
				working: '执行中…'
			},
			en: {
				...
				stageAllBtnHint: 'Add every change in the working tree — untracked files and deletions included — to the index: git add -A',
				unstageAllBtn: 'Unstage all',
				unstageAllBtnHint: 'Clear the index and put the changes back in the working tree: git restore --staged',
				stagedAllCount: 'Staged everything ({n} files)',
				stagedNone: 'Nothing to stage',
				unstagedAll: 'Unstaged all',
				conflictGroup: '{n} files in conflict',
				conflictLockedHint: 'Unresolved conflict: staging and unstaging are blocked until you resolve it (git add would record the conflict markers as the resolution)',
				conflictBlocked: 'Conflicts present — resolve them before staging in bulk',
				resolveConflicts: 'Let Agent resolve',
				resolveConflictsHint: "Hand the conflicts to this session's agent — it sees the full conversation context. It resolves conflicts only, and will not commit for you",
				resolvingDelegated: 'Conflicts handed to the session agent',
				stashPop: 'Pop stash',
				...
				commitBlockedHint: 'Unresolved conflicts: git refuses to commit — resolve them first, or use "Let Agent resolve" in the conflict group header',
				switching: 'Switching…',
				working: 'Working…'
			}
		}
...
function makeTheme() {
	return {
		 ...
		 */
		/**
		 * 组头右端 / 冲突横幅里的小动作按钮（全部暂存、全部取消暂存、Agent 解决冲突）。
		 * 不复用 Btn：它带边框、11px 字号，塞进 10px 的组头里既高又抢眼；
		 * 这类动作更像「链接式」按钮，所以单独做一个无边框的小号版本。
		 */
			...
			return h(
				'button',
				{
					...
					style: {
						...
						textTransform: 'none',
						// 用蓝色填充令牌当文字色：与主按钮同源，明暗主题自动跟随
						// （浅色约 4.2:1 / 深色约 7.1:1，与主按钮的对比度量级一致）。
						color: disabled ? theme.dim : props.color || theme.accent,
		...
		function StatusLetter(props) {
			// 颜色必须走 theme：这里曾经写成 `var t = props.t` 再读 `t.dim / t.del / t.add`，
			// 而字典里没有这些键 —— 于是 color 恒为 undefined，状态字母只能用继承色。
			// 与 Btn 那个「白字无底色」是同一类错误（见 docs/DESIGN.md 的已知坑）。
			var theme = props.theme
			var code = statusCodeOf(props.file)
			var color = theme.dim
			if (code === 'U' || code === 'D') color = theme.del
			else if (code === '?' || code === 'A') color = theme.add
			return h(
				'span',
				{
					style: {
		...
		var META_PREFIXES = [
			...
			'new mode',
			'Binary files',
			// 未解决冲突的 combined diff 头（UU / AA 走这条路，见 host 的 collectConflictDiff）
			'diff --cc'
		]
		...
		function GitTabBody(props) {
			...
			var stagedFiles = files.filter(function (f) {
				return !f.unmerged && f.staged
			})
			var otherFiles = files.filter(function (f) {
				return !f.unmerged && !f.staged
			})
			// 冲突单独成一组，排在列表最前面。注意 porcelain v2 里未解决条目是 `u UU ...`，
			// 而解析器的 `staged = x !== '.'` 会把它判成 staged:true —— 所以上面两个分组的
			// filter 都显式排除了 unmerged，否则冲突文件会同时冒进「已暂存」组。
			// 两条批量按钮也都要先看这份名单：实测 `git add -A` 会把未解决冲突直接标成
			// 已解决（状态变 `1 M.`），冲突标记就此进暂存区，只能改历史才能挽回。
			var conflictedFiles = files.filter(function (f) {
				return f.unmerged
			})
			var hasConflicts = conflictedFiles.length > 0
			// 选中的是不是冲突文件：冲突文件不提供单文件暂存 / 取消暂存（见 fileRow 的注释），
			// 详情区也就不该出现那个按钮。
			var selectedIsConflict =
				selected !== null &&
				conflictedFiles.some(function (f) {
			...
			function onStageAll() {
				run('stage-all', api('/stage-all', {})).then(function (value) {
					if (value) {
						ok(value.staged > 0 ? t.stagedAllCount.replace('{n}', String(value.staged)) : t.stagedNone)
			...
			function onUnstageAll() {
				run('unstage-all', api('/unstage-all', {})).then(function (value) {
					if (value) {
						ok(t.unstagedAll)
						return afterMutation()
					}
			...
			/**
			 * 把冲突交给当前会话的 Agent —— 与「交给 Agent 提交」同一条通道
			 * （binding.session.prompt，等价于用户自己打字），只是这次只要它解决冲突。
			 */
			function onResolveConflicts() {
				var binding = sessions.binding(sessionId)
				if (!binding || !binding.session) {
					report(new Error('no session binding'))
					return
				}
				var shown = conflictedFiles.slice(0, 10).map(function (f) {
					return f.path
				})
				var text =
					'当前工作区有 ' + conflictedFiles.length + ' 个文件处于冲突状态：' +
					shown.join('、') + (conflictedFiles.length > shown.length ? ' 等' : '') +
					'。请先解决这些冲突；只解决冲突，不要提交。'
				setError(null)
				binding.session
					.prompt([{ type: 'text', text: text }], 'queue')
					.then(function () {
						ok(t.resolvingDelegated)
					})
					.catch(report)
			}
			...
			function fileRow(file, isStaged, isConflict) {
				...
				return h(
					...
					h(StatusLetter, { theme: theme, file: file }),
					h(
						'span',
						{
							style: {
					...
					file.deletions !== null && file.deletions !== undefined
						...
						: null,
					// 冲突行给一个**不可点**的 `!`：不给 −/+。
					// `git add` 会把冲突标记当成解决结果（实测「先 − 再 +」之后
					// `<<<<<<<` 会被原样提交），而 `−` 正是把它送进「变更」组拿到 `+` 的那一步。
					isConflict
						? h(
								'span',
								{
									title: t.conflictLockedHint,
									style: {
										flex: '0 0 auto',
										padding: '0 4px',
										color: theme.del,
			...
			/**
			 * 分组标题行：左标签 + 右端动作。
			 * 没有动作时退化成原来的纯标签行。一行 flex：标签吃掉剩余宽度并省略号
			 * 截断，动作固定在右端 —— 因此组头本身仍然是 10px 那一行，不额外占高度。
			 */
			function sectionHeader(key, label, action) {
				if (!action) return h('div', { key: key, style: sectionStyle }, label)
				return h(
					'div',
					{
						key: key,
						style: Object.assign({}, sectionStyle, { display: 'flex', alignItems: 'center', gap: 6 })
					},
					h(
						'span',
						{
			...
			/**
			 * 冲突分组的组头：只在有未解决冲突时出现，且排在整个列表最前面。
			 *
			 * 它是唯一一个**不共用 `sectionHeader`** 的组头，因为它要同时说清两件事：
			 * 现在有冲突，以及唯一的出路（交给会话 Agent）。此时两条批量按钮都是禁用的
			 * （`add -A` 会把冲突标成已解决、`restore --staged` 会丢掉冲突状态），
			 * 所以这里不能只让按钮变灰、不给退路。因此组头带警示底色而不是普通 10px 灰字。
			 * 冲突文件行就挂在它下面（`fileRow(f, true)`，diff 请求形态与改动前一致）。
			 */
			function conflictHeader() {
				return h(
					...
					h(
						...
						t.conflictGroup.replace('{n}', String(conflictedFiles.length))
					),
					h(MiniBtn, {
						theme: theme,
						label: t.resolveConflicts,
			...
			function changesListContent() {
				return 					status === null
							...
							: files.length === 0
								...
								: [].concat(
										hasConflicts ? [conflictHeader()].concat(conflictedFiles.map(function (f) { return fileRow(f, false, true) })) : [],
										stagedFiles.length > 0
											? [sectionHeader('sh', t.staged, h(MiniBtn, { theme: theme, label: t.unstageAllBtn, title: hasConflicts ? t.conflictBlocked : t.unstageAllBtnHint, disabled: busy !== null || hasConflicts, onClick: onUnstageAll }))].concat(
											...
											: [],
										otherFiles.length > 0
											? [sectionHeader('ch', sectionLabel(otherFiles), h(MiniBtn, { theme: theme, label: t.stageAllBtn, title: hasConflicts ? t.conflictBlocked : t.stageAllBtnHint, disabled: busy !== null || hasConflicts, onClick: onStageAll }))].concat(
			...
			if (mode === 'changes') {
				...
				bottomAction = selected && !selectedIsConflict
					? h(Btn, {
							t: t, theme: theme,
					...
					? h(
						...
						h(
							...
							h(Btn, {
								...
								theme: theme,
								label: t.commit,
								// 有冲突时提交是物理上不可能的（git 拒绝），所以直接禁用并说明原因，
								// 而不是让用户点下去、再收到一条 git 的英文 hint/fatal。
								title: hasConflicts ? t.commitBlockedHint : t.commitHint,
								primary: true,
								busy: busy === 'commit',
