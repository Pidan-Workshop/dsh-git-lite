...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			return h(
				...
				h(
					...
					h(
						...
						{
							style: {
								flex: listHeight === null ? '1 1 0' : '0 0 ' + listHeight + 'px',
								minHeight: 0,
								overflow: 'auto'
							}
						},
		...
		function apply(ctx) {
			...
			/**
			 * 诊断：探测一个 seat 当前是否已声明。
			 *
			 * 存在的理由：slots.inject(seat, cb) 在 seat **尚未声明**时会把 cb 延后，
			 * 而延后执行的失败会走 queueMicrotask(() => { throw failure }) —— 抛出在
			 * microtask 里，插件的 try/catch 完全看不到（见 dsh-client-ui-renderer 的
			 * inject 实现）。真机排查时这段日志是唯一能区分「seat 未声明」「回调未执行」
			 * 「回调执行了但 register 抛错」的手段。
			 */
			function probe(scope, seat) {
				try {
					var bySpec = typeof scope.slots.spec === 'function' ? scope.slots.spec(seat) : undefined
					var byDynamic =
						typeof scope.slots.specDynamic === 'function' ? scope.slots.specDynamic(seat) : undefined
					return (
						'spec=' + (bySpec === undefined ? 'NONE' : String(bySpec.kind)) +
						' specDynamic=' + (byDynamic === undefined ? 'NONE' : String(byDynamic.kind))
					)
				} catch (err) {
					return 'probe-threw: ' + (err && err.message)
				}
			}

			/** 注册一个 keyed seat 的占用者，并逐段打点。 */
			function claimSeat(scope, seat, options, component, label) {
				scope.slots.inject(seat, function () {
					console.log('[git-lite] ' + label + ': seat callback fired')
					var dispose = scope.slots.register(Object.assign({ name: seat }, options), component)
					console.log('[git-lite] ' + label + ': registered, disposer=' + typeof dispose)
					return dispose
				})
			}
			...
			ctx.inject(['sidebarRightTabs'], function (scope) {
				try {
					...
					console.log('[git-lite] tab type registered (id=' + TAB_ID + ' kind=' + TAB_KIND + ')')
				} catch (err) {
					console.warn('[git-lite] tab type registration failed', err)
				}
			})
			...
			ctx.inject(['slots', 'sessions', 'locale'], function (scope) {
				console.log(
					'[git-lite] pane seat at inject time: ' + probe(scope, 'sidebar.right.pane.tab')
				)
				try {
					claimSeat(
						scope,
					...
					console.warn('[git-lite] pane body inject failed', err)
				}
				try {
					claimSeat(
						scope,
					...
					console.warn('[git-lite] pane title inject failed', err)
				}
			})
			...
			ctx.inject(['slots', 'sessions', 'locale', 'sidebarRight'], function (scope) {
				console.log(
					'[git-lite] header seat at inject time: ' +
						probe(scope, 'conversation.session.header.utilities')
				)
				try {
					claimSeat(
						scope,
					...
					console.warn('[git-lite] header chip inject failed', err)
				}
			})
		}
