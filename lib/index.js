 ...
 *
 * 三道校验，任何一道不过就拒绝：
 *   1. sessionId 必须对应一个活着的会话，且该会话有 cwd；
 *   2. cwd 必须落在一个已注册工作区内；
 ...
 */
export async function resolveRepo(ctx, sessionId) {
	...
	const workspace = await ctx.workspaceRegistry.resolveByPath(canonicalCwd)
	if (workspace === undefined) {
		fail('workspace-unknown', 'working directory is not a registered workspace')
	}
...
function withRepo(ctx, handler) {
	return async (body) => {
		const repo = await resolveRepo(ctx, body.sessionId)
