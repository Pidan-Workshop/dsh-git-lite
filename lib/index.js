...
/**
 * 未解决冲突（unmerged）的文件路径。
 *
 * 批量暂存前必须先看这份名单：`git add -A` 会把未解决的冲突**直接标记为已解决**，
 * 冲突标记就此进入暂存区 —— 那不是「多暂存了几个文件」这种可以取消暂存的后果，
 * 而是只能改历史才能挽回。所以凡是会跑 `add -A` 的路径（/stage-all 与
 * /commit 的 stageAll 分支）都要先过这道闸门。
 *
 * 抽成纯函数是为了能直接单测：它只需要一份 parseStatus 的结果。
 */
export function conflictPaths(status) {
	return status.files.filter((f) => f.unmerged).map((f) => f.path)
}

/**
 * 该路径是否处于未解决冲突。
 *
 * 单文件的两个动作和批量一样危险，所以同样要过闸门：
 *   - `/stage`（行内 `+`）：`git add <unmerged>` 会把冲突**标记为已解决**。实测
 *     「先点 `−` 再点 `+`」之后，工作区里的 `<<<<<<< HEAD ... ======= ... >>>>>>>`
 *     会被原样提交进历史；
 *   - `/unstage`（行内 `−`）：`git restore --staged` 把未解决条目标回 HEAD（丢掉冲突
 *     状态），而文件随即掉进「变更」组、拿到一个 `+` —— 正好接上上面那条路。
 * 客户端已经不再给冲突行这两个按钮，这里是纵深防御（与批量闸门同一个 unmerged 概念）。
 */
export function isUnmergedPath(path, status) {
	return status.files.some((f) => f.unmerged && f.path === path)
}

/** 单文件暂存 / 取消暂存被冲突拦下时的文案。 */
function pathConflictedMessage(path, what) {
	return `「${path}」处于冲突状态：${what}。请先解决冲突。`
}

/**
 * git 对某些未解决冲突**不提供任何差异内容**，只回这一行占位文本 ——
 * 实测 modify/delete（UD / DU）就是如此，三条 diff 命令都一样。
 */
const UNMERGED_PLACEHOLDER = /^\* Unmerged path /

/**
 * 从 `git ls-files -u -- <path>` 的输出判断冲突形态。
 * stage 1 = 共同祖先，2 = 本分支（ours），3 = 对方（theirs），于是：
 *   {2,3}   → AA（双方各自新增，没有共同祖先）
 *   {1,2,3} → UU（两边都改了同一段）
 *   {1,2}   → UD（本分支改了、对方删了）
 *   {1,3}   → DU（本分支删了、对方改了）
 * 抽成纯函数是为了能直接单测：喂一段真实的 ls-files 输出即可。
 */
export function conflictShape(lsFilesU) {
	const stages = new Set()
	for (const line of lsFilesU.split('\n')) {
		if (line === '') continue
		const parts = line.split('\t')[0].split(' ')
		if (parts.length >= 3) stages.add(parts[2])
	}
	if (stages.size === 0) return null
	if (stages.has('2') && stages.has('3')) return stages.has('1') ? 'UU' : 'AA'
	if (stages.has('2')) return 'UD'
	if (stages.has('3')) return 'DU'
	return null
}

/**
 * 冲突说明文本：git 给不出差异时用它顶上。
 *
 * 面板里留一片空白，用户只会以为「插件坏了」；所以要说清这是什么冲突、
 * 以及有哪两条出路。**面板不替用户选边**，冲突行也不提供暂存按钮，
 * 所以这里给的是终端命令（或交给 Agent）。
 */
export function conflictDiffNote(path, shape) {
	if (shape === 'UD' || shape === 'DU') {
		return [
			`冲突尚未解决：modify/delete（${shape}）`,
			'一边修改了这个文件，另一边删除了它 —— git 对这种冲突不提供差异内容。',
			'',
			shape === 'UD' ? '  UD = 本分支修改、对方删除' : '  DU = 本分支删除、对方修改',
			'',
			'要选一边收场（面板不替你做这个决定）：',
			`  · 保留文件 → git add ${path}`,
			`  · 接受删除 → git rm ${path}`
	...
	return [
		`冲突尚未解决（${shape === null ? 'unmerged' : shape}）`,
		'',
		'git 没有为此文件提供差异内容。解决冲突后它会恢复正常显示。'
	].join('\n')
}

/** 冲突拒绝时的统一文案（三个入口共用，避免几处说法不一致）。 */
function conflictedMessage(n, what) {
	return `有 ${n} 个文件处于冲突状态，${what}。请先解决冲突。`
}
 ...
 *
 * 这道闸门是实测出来的，不是想当然：
 *   - `git add -A` 会把未解决冲突**直接标记为已解决**（临时仓库验证：状态从
 *     `u UU ...` 变成 `1 M. ...`），冲突标记就此进入暂存区，只能改历史才能挽回；
 *   - 反向的 `restore --staged` 会把未解决条目标回 HEAD，等于丢掉冲突状态。
 * 两者都不是「多暂存了几个文件」这种可以撤销的后果，所以先拒绝。
 *
 * 判定与 UI 共用同一份 collectStatus（即同一个 unmerged 概念），不另起一套
 * `--diff-filter=U`，避免宿主与面板对「什么算冲突」理解不一致。
 */
async function assertNotConflicted(root, what) {
	const conflicts = conflictPaths(await collectStatus(root))
	if (conflicts.length > 0) fail('conflicted', conflictedMessage(conflicts.length, what))
}
 ...
 * 冲突文件的 diff。
 *
 * `git diff` 对 UU / AA 会给出带 `<<<<<<<` 标记的 combined diff（`diff --cc`），
 * 那正是我们要给用户看的东西；对 UD / DU 只回一行 `* Unmerged path`，那就换成
 * 一段说明（conflictDiffNote）。
 *
 * 冲突文件一律走**工作区** diff：`--cached` 对任何冲突都只回占位文本 ——
 * 这正是面板原来只显示「* Unmerged path xxx」的原因。
 */
async function collectConflictDiff(root, path, shape, maxBytes) {
	const worktree = await runGit(['diff', '--no-color', '-U3', '--', path], root, {
		maxBuffer: DIFF_MAX_BUFFER
	})
	const text = worktree.ok ? worktree.stdout : ''
	if (text.trim() !== '' && !UNMERGED_PLACEHOLDER.test(text.trim())) {
		return clipDiff(text, maxBytes)
	}
	return { diff: conflictDiffNote(path, shape), truncated: false }
}
...
/**
 * 组装一条符合 dsh-llm 契约的用户消息。
 *
 * 契约要求 content 是**块数组**（`[{ type: 'text', text }]`）而不是裸字符串，
 * 且消息要带 id 与 source。传裸字符串会在适配器里直接炸掉
 * （`serializeMessages` 会对 content 调 `.filter()`），而那个失败又会被包成
 * 终止分片 —— 若消费方不读终止分片，症状就只剩一句「模型没有返回提交信息」。
 */
...
/**
 * 收集 llm.stream 的文本增量，并把**终止分片里的失败抛出来**。
 *
 * 每个流都以一个终止分片结束：`{ type: 'finish', reason }`，其中 reason.kind 为
 * `stop` / `error` / `aborted`。任何失败（适配器抛错、模型返回空内容、上下文超限、
 * 限额、凭据错误）都走 `reason.kind === 'error'`，真正的诊断在 `reason.failure`
 * （稳定 code + message）。
 *
 * 这里必须显式抛出：只挑 text-delta 会把失败整条吞掉，只剩下空字符串，最后被
 * 上层报成「模型没有返回提交信息」—— 一句误导人的话，把真正的原因盖住了。
 */
export async function streamToText(iterable) {
	let text = ''
	for await (const chunk of iterable) {
		...
		fail(
			typeof failure.code === 'string' && failure.code !== ''
				? failure.code
				: aborted ? 'llm-aborted' : 'llm-failed',
			typeof failure.message === 'string' && failure.message !== ''
				? failure.message
				: aborted ? '模型调用被取消' : '模型调用失败'
		)
	}
	return text.trim()
}
...
function createRouter(ctx, config, llmRef) {
	...
	const routes = {
		...
		[`${API}/diff`]: withRepo(ctx, async (repo, body) => {
			...
			// 未解决冲突走单独一条路（原因见 collectConflictDiff）：索引里有未合并的
			// stage 就说明这个路径处于冲突中，`git ls-files -u` 非空即为准。
			const unmerged = await mustGit(['ls-files', '-u', '--', path], repo.root)
			if (unmerged.trim() !== '') {
				return collectConflictDiff(repo.root, path, conflictShape(unmerged), config.maxDiffBytes)
			}
		...
		[`${API}/stage`]: withRepo(ctx, async (repo, body) => {
			const path = assertSafePath(body.path)
			// 冲突文件不能暂存：git add 会把冲突标记当成解决结果。
			if (isUnmergedPath(path, await collectStatus(repo.root))) {
				fail('conflicted', pathConflictedMessage(path, '暂存它会把冲突标记当成解决结果'))
			}
		...
		[`${API}/unstage`]: withRepo(ctx, async (repo, body) => {
			const path = assertSafePath(body.path)
			// 冲突文件不能取消暂存：会把未解决条目标回 HEAD，丢掉冲突状态。
			if (isUnmergedPath(path, await collectStatus(repo.root))) {
				fail('conflicted', pathConflictedMessage(path, '取消暂存会丢掉冲突状态'))
			}
		...
		[`${API}/stage-all`]: withRepo(ctx, async (repo) => {
			await assertNotConflicted(repo.root, '全部暂存会把它们标记为已解决')
			await mustGit(['add', '-A'], repo.root)
			// 以「暂存区里现在有多少文件」作为反馈：为 0 时客户端提示「没有可暂存的改动」。
			const staged = await mustGit(['diff', '--cached', '--name-only'], repo.root)
			return { staged: staged.split('\n').filter((line) => line !== '').length }
		}),

		[`${API}/unstage-all`]: withRepo(ctx, async (repo) => {
			await assertNotConflicted(repo.root, '全部取消暂存会把冲突条目回退掉')
			// 与单文件 /unstage 同一套两步：尚无提交（unborn HEAD）时
			// `git restore --staged` 必然失败（实测 fatal: could not resolve HEAD），
			// 而 `git reset --quiet` 在那种仓库里 exit 0 且能清空索引。
			const restored = await runGit(['restore', '--staged', '--', '.'], repo.root)
			if (!restored.ok) {
				await mustGit(['reset', '--quiet'], repo.root)
			}
			return { ok: true }
		}),
		...
		[`${API}/commit`]: withRepo(ctx, async (repo, body) => {
			...
			// 未解决冲突时 git 一律拒绝提交，而它回的是英文 hint/fatal 长句 ——
			// 不该原样丢给用户（与 /pull 的分叉提示同一原则）。所以这里先拦下，
			// 且**两个分支都要盖到**（勾没勾「全部暂存」都一样提交不了）。
			//
			// 这道闸门必须放在「暂存区为空」之前：实测 `git diff --cached --name-only`
			// **会**把未解决条目列出来，所以那道检查挡不住冲突 —— 真机上就是这样漏出去的：
			// 面板把 `error: Committing is not possible because you have unmerged files.`
			// 连同 hint/fatal 原文显示在了红条里。
			await assertNotConflicted(repo.root, '无法提交')
